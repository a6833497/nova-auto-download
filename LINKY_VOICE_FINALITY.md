...
1. `linke_ledger_pull.py GUILD YYYYMMDD` reads every raw API page. Zero-value rows do not terminate pagination. It atomically archives response bodies and a checksum under `LINKE_EVIDENCE_DIR` before the existing ledger upsert. Authorization headers, cookies and BI tickets are never persisted. The directory is mode `0700`, files are `0600`, and matching evidence files are retained for 14 days by default (`LINKE_EVIDENCE_RETENTION_DAYS`).
